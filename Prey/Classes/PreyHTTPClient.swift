...
//
//  forked from https://github.com/Alamofire/Alamofire

import Foundation

struct PreyHTTPClient {
...
/// Errors
public let ErrorDomain = "error.preyproject.com"
...
public class Manager {
    ...
    /**
    A shared instance of `Manager`, used by top-level request methods, and suitable for use directly for any ad hoc requests.
    */
    ...
    */
    ...
    class TaskDelegate: NSObject, NSURLSessionTaskDelegate {
        ...
        init(task: NSURLSessionTask) {
            ...
            self.queue = {
                let label: String = "com.prey.task-\(task.taskIdentifier)"
        ...
        dispatch_async(delegate.queue) {
            if self.response != nil && self.delegate.error == nil {
                if !validation(self.request, self.response!) {
                    self.delegate.error = NSError(domain: ErrorDomain, code: -1, userInfo: nil)
                }
            }
        }
