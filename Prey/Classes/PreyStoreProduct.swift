...
class PreyStoreProduct {
    ...
    func verifyReceiptOnComplete(_ onComplete:@escaping ()->Void, onError:@escaping () -> Void) {
        ...
        if let username = PreyConfig.sharedInstance.userApiKey {
            ...
            PreyHTTPClient.sharedInstance.userRegisterToPrey(username, password:"x", params:params, messageId:nil, httpMethod:Method.POST.rawValue, endPoint:subscriptionEndpoint, onCompletion:PreyHTTPResponse.checkResponse(RequestType.subscriptionReceipt, preyAction:nil, onCompletion:{(isSuccess: Bool) in
