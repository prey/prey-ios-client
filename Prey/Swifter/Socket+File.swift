    ...
    private func sendfileImpl(_ source: UnsafeMutablePointer<FILE>, _ target: Int32, _: off_t, _: UnsafeMutablePointer<off_t>, _: UnsafeMutablePointer<sf_hdtr>, _: Int32) -> Int32 {
        ...
        while true {
            ...
            while writeCounter < readResult {
                let writeResult = buffer.withUnsafeBufferPointer { bufferPointer in
                    let offsetPointer = bufferPointer.baseAddress! + writeCounter
                    #if os(Linux)
                        return send(target, offsetPointer, readResult - writeCounter, Int32(MSG_NOSIGNAL))
                    #else
                        return write(target, offsetPointer, readResult - writeCounter)
                    #endif
                }
